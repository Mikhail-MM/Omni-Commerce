...
exports.login = async (req, res, next) => {
  try {
    ...
    if (!req.body.email) {
      return res
        .status(404)
        .send('Please input your Email to log in.');
    }
    if (!req.body.password) {
      return res
        .status(404)
        .send('Please input your Password to log in.');
    }

    const userCollection = req.body.loginPath === 'omni' ? OmniUser : EssosUser;

    const authorizedClient = await userCollection.findOne({
      email: req.body.email,
    });
    if (!authorizedClient) {
      return res
        .status(404)
        .send('Account with this email does not exist');
    }
...
async function validateToken(req, res, next, authReq) {
  try {
    ...
    try {
      ...
      const validatedClient = await userCollection.findById(
        decoded._id,
      );

      if (!validatedClient) return res.status(403).send('Invalid Client.');
      if (token !== validatedClient.token) return res.status(403).send('Expired Token');
      ...
      if (timeDifference >= 2) {
        return res
          .status(403)
          .send(
            'Token expired after 2 hours elapsed, please log-in again to refresh your token.',
          );
      }
      ...
      if (
        !validatedClient.isAdmin
        && !validatedClient.isSuperAdmin
        && authReq.adminRequired
      ) return res.status(403).send('Admin privileges required');

      if (
        !validatedClient.isSuperAdmin
        && authReq.superAdminRequired
      ) {
        return res
          .status(403)
          .send('Super Admin privileges required');
      }
...
exports.routeEmployeeToMongoCollection = function (req, res, next) {
  validateToken(req, res, next, {
    attachMongoCollectionKeyHeaders: true,
  });
};
