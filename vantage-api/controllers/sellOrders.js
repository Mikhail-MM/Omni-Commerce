...
const { sellerSpecificPurchaseOrderModel } = MarketplaceModels;
...
module.exports.getSellOrderById = async function (req, res, next) {
  try {
    const sellOrder = await sellerSpecificPurchaseOrderModel.findOne({
      _id: req.params.id,
    });
    res.json(sellOrder);
  } catch (err) {
...
module.exports.getMySellOrders = async function (req, res, next) {
  try {
    const sellOrdersByUser = await sellerSpecificPurchaseOrderModel.find(
...
module.exports.updateStatus = async (req, res, next) => {
  try {
    const sellOrderToUpdate = await sellerSpecificPurchaseOrderModel
      .findById(req.body.sellOrderId)
      .lean();
    const purchaseOrderToUpdate = await PurchaseOrderModels.findById(
      sellOrderToUpdate.masterPurchaseOrderRef_id,
    ).lean();

    const sellOrderSubdocs = sellOrderToUpdate.itemsBought;
    // FrontEnd passes the unique _id of each subdoc item within the sell order
    const itemToUpdate = sellOrderSubdocs.find(
      (item) => item._id == req.body.itemSubdocId,
    );
    const indexToUpdate = sellOrderSubdocs.findIndex(
      (item) => item._id == req.body.itemSubdocId,
    );
    ...
    const purchaseOrderSubdocs = purchaseOrderToUpdate.itemsBought;
    // FrontEnd does not have access to the purchase order subdoc raw ID - need to look for itemRef Id
    const recieverItemToUpdate = purchaseOrderSubdocs.find(
      (receivedItem) => receivedItem.itemRef_id.toString()
        === newItemInstance.itemRef_id.toString(),
    );
    const recieverIndexToUpdate = purchaseOrderSubdocs.findIndex(
      (item) => item.itemRef_id.toString()
        === newItemInstance.itemRef_id.toString(),
    );

    const newReceivedItemInstance = {
      ...recieverItemToUpdate,
      status: req.body.status,
    };

    purchaseOrderSubdocs[
      recieverIndexToUpdate
    ] = newReceivedItemInstance;
