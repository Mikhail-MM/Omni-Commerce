...
module.exports.tabulateDailyTicketSales = async function (
  req,
  res,
  next,
) {
  ...
  const DailySalesReports = mongoose.model(
    'SalesReport',
    SalesReportSchema,
    `${req.headers['x-mongo-key']}_SalesReports`,
  );
  try {
    const allTicketsBySession = await Transaction.find({});
    const beginDate = new Date(allTicketsBySession[0].createdAt);
    const endDate = new Date(
      allTicketsBySession[allTicketsBySession.length - 1].createdAt,
    );

    const allTicketsByCategory = _.groupBy(
      allTicketsBySession,
      'status',
    );
    ...
    const allItemsSoldByItem = _.groupBy(
      allPaidMenuItems,
      'itemName',
    );

    console.log('all Items by Item', allItemsSoldByItem);
    const allItemsSoldByCategory = _.groupBy(
      allPaidMenuItems,
      'category',
    );

    const grossSalesByItemName = {};
    const grossSalesByServer = {};
    const grossSalesByCategory = {};

    const menuItemGross = Object.keys(allItemsSoldByItem).map(
      (itemNameKey) => {
        grossSalesByItemName[itemNameKey] = allItemsSoldByItem[
          itemNameKey
        ]
          .map(
            (subdocItemsByName) => new BigNumber(subdocItemsByName.itemPrice),
          )
          .reduce((acc, cur) => acc.plus(cur))
          .toNumber();
        ...
        return {
          dataKey: itemNameKey,
          dataValue: grossSalesByItemName[itemNameKey],
        };
      },
    );

    const employeeGrossMetrics = Object.keys(allTicketsByServer).map(
      (employeeNameKey) => {
        grossSalesByServer[employeeNameKey] = allTicketsByServer[
          employeeNameKey
        ]
          .map(
            (employeeTickets) => new BigNumber(employeeTickets.totalReal),
          )
          .reduce((acc, cur) => acc.plus(cur))
          .toNumber();

        return {
          dataKey: employeeNameKey,
          dataValue: grossSalesByServer[employeeNameKey],
        };
      },
    );

    const categoryMetrics = Object.keys(allItemsSoldByCategory).map(
      (categoryKey) => {
        grossSalesByCategory[categoryKey] = allItemsSoldByCategory[
          categoryKey
        ]
          .map(
            (subdocItemsByCategory) => new BigNumber(subdocItemsByCategory.itemPrice),
          )
          .reduce((acc, cur) => acc.plus(cur))
          .toNumber();
        ...
        return {
          dataKey: categoryKey,
          dataValue: grossSalesByCategory[categoryKey],
        };
      },
    );

    const timeSeriesData = allPaidTickets.map((ticket) => ({
...
module.exports.lookupByTimestamp = async function (req, res, next) {
  ...
  try {
    const arrayOfSalesReports = await DailySalesReports.find({
      $and: [
        { beginDate: { $gte: req.body.beginDate } },
    ...
    const allTicketsBySession = arrayOfSalesReports
      .map((salesReportData) => salesReportData.tickets)
      .reduce((acc, cur) => acc.concat(cur));
    ...
    const allMenuItemsSoldByItem = _.groupBy(
      allMenuItemsSoldAndPaidFor,
      'itemName',
    );
    const allMenuItemsSoldByCategory = _.groupBy(
      allMenuItemsSoldAndPaidFor,
      'category',
    );
    ...
    const menuItemsGross = Object.keys(allMenuItemsSoldByItem).map(
      (key) => {
        grossSalesByMenuItemObject[key] = allMenuItemsSoldByItem[key]
          .map(
            (arrayOfSoldMenuItemsInCategory) => new BigNumber(arrayOfSoldMenuItemsInCategory.itemPrice),
          )
          .reduce((acc, cur) => acc.plus(cur))
          .toNumber();
        ...
        return {
          dataKey: key,
          dataValue: grossSalesByMenuItemObject[key],
        };
      },
    );

    const serverTallyGross = Object.keys(allTicketsByServer).map(
      (key) => {
        grossSalesByServer[key] = allTicketsByServer[key]
          .map(
            (arrayOfSoldMenuItemsInCategory) => new BigNumber(arrayOfSoldMenuItemsInCategory.totalReal),
          )
          .reduce((acc, cur) => acc.plus(cur))
          .toNumber();
    ...
    const menuItemCategoryGross = Object.keys(
      allMenuItemsSoldByCategory,
    ).map((key) => {
      grossSalesByMenuItemCategory[key] = allMenuItemsSoldByCategory[
        key
      ]
        .map(
          (arrayOfSoldMenuItemsInCategory) => new BigNumber(arrayOfSoldMenuItemsInCategory.itemPrice),
      ...
      return {
        dataKey: key,
        dataValue: grossSalesByMenuItemCategory[key],
      };
    });
...
module.exports.getSalesReportById = async function (req, res, next) {
  ...
  try {
    const salesReport = await DailySalesReports.findOne({
      _id: req.params.id,
    });
