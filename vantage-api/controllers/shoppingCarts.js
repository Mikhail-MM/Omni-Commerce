...
module.exports.validateCartStock = async function (req, res, next) {
  try {
    ...
    const passedItems = [];
    const failedItems = [];
    let partialValidationFail = false; // change to false and attach to response
...
module.exports.calculatePricing = async function (req, res, next) {
  ...
  const subTotalBigNumber = bigNumberPrices.reduce((acc, cur) => acc.plus((cur.itemPrice.times(cur.multipleRequest))), new BigNumber(0));
  ...
  } if (req.body.validatedPurchaseOrderToProcess) {
