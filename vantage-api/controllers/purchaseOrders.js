...
module.exports.createNewPurchaseOrder = async function (
  req,
  res,
  next,
) {
...
module.exports.updatePurchasOrderById = async function (
  req,
  res,
  next,
) {
  try {
    const updatedPurchaseOrder = await PurchaseOrderModel.findOneAndUpdate(
      { _id: req.params.id },
      req.body,
      { new: true },
    );
...
module.exports.getMyPurchaseOrders = async function (req, res, next) {
  try {
    const purchaseOrdersByUser = await PurchaseOrderModel.find({
