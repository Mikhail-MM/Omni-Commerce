...
module.exports.getProfileMetadata = async function (req, res, next) {
  const profileInfo = await EssosUser.findById(
    req.params.id,
    'firstName lastName avatarURL followers following wishlist',
  );
    ...
    if (!newClient.hash) {
      ...
      if (req.body.accountType === 'OnlineMerchant') {
        ...
        const updatedMarketplaceWithClientRef = await MarketplaceModel.findOneAndUpdate(
          { _id: response.createdMarketplace._id },
          { ownerRef_id: registeredClient._id },
          { new: true },
        );
...
module.exports.autoCompleteClientOrgName = function (req, res, next) {
  ...
  Client.find(
    {
      organizationName: {
        $regex: `^${req.body.employerLookup}i`,
      },
    },
    'organizationName',
    (err, clients) => {
...
module.exports.findMasterAndTagChild = async function (
  req,
  res,
  next,
) {
...
module.exports.updateClient = async function (req, res, next) {
  try {
    const User = req.headers['x-user-pathway'] === 'Essos'
      ? EssosUser
      : OmniUser;
