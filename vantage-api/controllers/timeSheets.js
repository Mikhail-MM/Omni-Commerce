...
module.exports.getAllTimesheets = async (req, res, next) => {
  try {
    const TimeSheetModel = mongoose.model(
      'TimeSheet',
      TimeSheetSchema,
      `${req.headers['x-mongo-key']}_TimeSheets`,
    );
...
module.exports.createNewTimesheet = async (req, res, next) => {
  try {
    console.log(
      'Creating New Timesheet. Clock In number:',
      req.body.clockInNumber,
    );

    const UserClockingIn = await OmniUser.findOne({
      $and: [
        { terminalIDNumber: req.body.clockInNumber },
        { mongoCollectionKey: req.headers['x-mongo-key'] },
      ],
    });

    console.log(
      'Found Client with that Terminal Number: ',
      UserClockingIn,
    );
    console.log(
      'Retreiving full name of User and appending to req.body.name:',
    );
    ...
    console.log(
      'Appending newly saved Timesheet Doc. to req.body.timesheet: ',
      savedTimeSheet,
    );
...
module.exports.checkForMissedTimesheets = async (req, res, next) => {
  try {
    ...
    if (LookForAndUpdateForgottenTimesheet) {
      ...
      console.log(
        'Missed Clock-Out Updated - Logged',
        LookForAndUpdateForgottenTimesheet,
      );
    }
...
module.exports.clockOutEmployee = async (req, res, next) => {
  try {
    console.log(
      'Closing Timesheet. Clock In number:',
      req.body.clockInNumber,
    );
    ...
    if (ClosedTimeSheet) {
      console.log(
        'Logging and attaching ClosedTimeSheet to req.body.timeSheet',
      );
    }
