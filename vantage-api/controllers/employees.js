...
module.exports.findMyEmployees = async function (req, res, next) {
  try {
    ...
    const myEmployees = await OmniUser.find({
      mongoCollectionKey: req.headers['x-mongo-key'],
    });
    if (!myEmployees) {
      return res
        .status(404)
        .send('Could not find any employees in this organization');
    }
...
module.exports.approveEmployeeSignUp = async function (
  req,
  res,
  next,
) {
  try {
    const employeeToUpdate = await Client.findOneAndUpdate(
      { _id: req.body._id },
      { employeeAuthorization: true },
      { new: true },
    );
    if (!employeeToUpdate) {
      return res
        .status(404)
        .send('Could not find the employee you were looking for');
    }
...
module.exports.disableEmployeeAccess = async function (
  req,
  res,
  next,
) {
...
module.exports.approveEmployeeManagementAdminPermissions = async function (
  req,
  res,
  next,
) {
...
module.exports.disableEmployeeManagementAdminPermissions = async function (
  req,
  res,
  next,
) {
...
module.exports.sendPrivateEmployeeInvitation = async function (
  req,
  res,
  next,
) {
