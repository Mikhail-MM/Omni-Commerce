...
const { storeConfigSchema } = storeConfig;
...
const { ShoppingCartModel } = MarketPlaceModels;
...
module.exports.registerOmniMaster = async (req, res, next) => {
  try {
    ...
    const StoreConfig = mongoose.model(
      'StoreConfig',
      storeConfigSchema,
      `${req.body.mongoCollectionKey}_StoreConfig`,
    );
...
module.exports.registerOmniChild = async (req, res, next) => {
  try {
    ...
    if (!masterAccount) {
      console.log(
        'Master Account does not exist - Delete client and abort ! ',
      );
      return res
        .status(400)
        .send(
          'Could not find a Master account with that organization name - Retry please',
        );
    }
    ...
    const iteratedBoss = await OmniUser.findByIdAndUpdate(
      masterAccount._id,
      { employeeCounter: iteratedBossEmployeeCounter },
      { new: true },
    );
    res.json({
      user: savedEssosUser,
...
createTerminalAccount = async function (req, res, next) {
  try {
    ...
    const newTerminal = new Client(data);
    if (!newTerminal.hash) {
...
updateEmployerEmployeeCount = function (req, res, next) {
  Client.findByIdAndUpdate(
    req.body.master_id,
    { employeeCounter: req.body.newEmployeeCount },
    { new: true },
    (err, newBoss) => {
