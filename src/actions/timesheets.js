...
export function clockEmployeeOut(token, employeeNumber) {
  ...
  return (dispatch) => fetch(`${hostURI}/timesheets/co`, {
    headers: {
    ...
    .then((json) => {
      ...
      console.log(json);
      return dispatch(fetchLoggedUsers(token));
    }); // Dispatch action to Redux Store informing Client of array of currently clocked in users to enter orders. Analyze Populated Query
}
...
export function clockEmployeeIn(token, employeeNumber) {
  ...
  console.log(data);
  return (dispatch) => fetch(`${hostURI}/timesheets/ci`, {
    headers: {
      'Content-Type': 'application/json',
      'x-access-token': token,
    },
