...
export function retrieveShoppingCart(token) {
  ...
  console.log(token);
  return (dispatch) => fetch(`${hostURI}/shoppingCart/userLookup/`, {
    headers: {
...
export function pushItemIntoShoppingCart(
  token,
  itemId,
  amountRequested,
  amountAlreadyInCart,
  ...
  console.log(
    'amountRequested, ammountAlreadyInCart: ',
    amountRequested,
    amountAlreadyInCart,
  );
  ...
  return (dispatch) => {
    fetch(url, {
      ...
      .then((json) => {
        ...
        if (
          amountAlreadyInCart + amountRequested
          > json.numberInStock
        ) {
          console.log(
            'Setting cart quantity to max SKU fulfillable by merchant',
          );
          ...
          console.log(
            'amountThatCanBeFulfilled, # of item which can not be added:',
            amountThatCanBeFulfilled,
            unfulfillable,
          );
          ...
          if (amountThatCanBeFulfilled > 0) {
            ...
            return fetch(`${hostURI}/shoppingCart/addItem`, {
              ...
              .then((pushedCart) => {
                console.log(
                  "Updating client's shopping cart with updated version",
                );
                ...
                dispatch(
                  receiveInvalidatedShoppingCartItems(
                    unfulfillableObject,
                  ),
                );
          ...
          console.log(
            'If statement running: Item DB is greater than or equal to amount requested + amount already in cart',
          );
          return fetch(`${hostURI}/shoppingCart/addItem`, {
            headers: {
              'Content-Type': 'application/json',
            ...
            .then((cartJson) => {
              dispatch(
                showModal('CART_ADDITION_SUCCESS_MODAL', {
                  itemName: json.itemName,
                }),
              );
...
export function validateCartAndProceedToPayment(token) {
  return (dispatch) => {
    // check validation, watch for flags, dispatch to payment gateway or just bring up a modal
    fetch(`${hostURI}/shoppingCart/checkOut/`, {
      ...
      .then((json) => {
        if (json.partialValidationFail) {
          dispatch(
            receiveInvalidatedShoppingCartItems(json.failedItems),
          );
