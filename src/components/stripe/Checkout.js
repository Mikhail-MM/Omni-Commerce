...
import {
  sendStripeTokenToApi,
  beginCartPaymentValidationCascade,
...
const mapDispatchToProps = (dispatch) => ({
  ...
  sendEssosStripeTokenToApi: (authToken, stripeToken) => {
    dispatch(
      beginCartPaymentValidationCascade(authToken, stripeToken),
    );
  },
});
...
const PaymentCheckoutForm = (props) => {
  ...
  const handleSubmit = (event) => {
    ...
    if (props.apiStripePath === 'Omni') {
      props.stripe
        .createToken({ name: 'Random Customer' })
        .then(({ token }) => {
          sendOmniStripeTokenToApi(
            authToken,
            token,
            activeTicket._id,
          );
...
const ConnectedInjectedCheckoutForm = injectStripe(
  connect(mapStateToProps, mapDispatchToProps)(PaymentCheckoutForm),
);
  ...
  <StripeProvider apiKey={stripeAPIkey}>
    <Elements>
      <ConnectedInjectedCheckoutForm
        apiStripePath={props.apiStripePath}
      />
    </Elements>
  </StripeProvider>
);
