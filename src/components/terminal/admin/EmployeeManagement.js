...
class EmployeeManagement extends Component {
  ...
  async componentDidMount() {
    ...
    const getEmployeesFunctionReturn = await this.findMyEmployees(
      token,
    );
    this.setState({
    ...
    return myEmployees.map((employee) => {
      ...
      if (!this.state.workerSearch) {
        return (
          <tr
            onClick={() =>
              this.props.showModal('OMNI_EMPLOYEE_MANAGEMENT_MODAL', {
        ...
        if (
          fullName
            .toLowerCase()
            .includes(this.state.workerSearch.toLowerCase())
        ) {
          return (
            <tr
              onClick={() =>
                this.props.showModal(
                  'OMNI_EMPLOYEE_MANAGEMENT_MODAL',
                  { profileData: employee },
                )
              }
            >
        ...
        <div className="employee-actions-bar">
          ...
          <div className="admin-employee-search-bar-container">
            ...
            <input
              style={{ flexGrow: 1 }}
              type="text"
              value={this.state.workerSearch}
              onChange={(e) =>
                this.handleChange('workerSearch', e.target.value)
              }
            />
          </div>
        </div>
