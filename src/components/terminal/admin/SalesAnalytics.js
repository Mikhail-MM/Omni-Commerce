...
const mapDispatchToProps = (dispatch) => ({
  loadSalesReport: (token, id) =>
    dispatch(getSalesReportById(token, id)),
});
...
class SalesAnalyticsContainer extends Component {
  ...
  renderSaleReportTableToDOM = () => {
    ...
    return allReports.map((report) => {
      return (
        <tr
          onClick={() =>
            this.props.loadSalesReport(token, report._id)
          }
        >
          <td>
            {' '}
            {moment(report.beginDate).format(
  ...
  renderChartsToDOM = () => {
    ...
    const formatCategoryPie = salesReport.categoryMetrics.map(
      (categoryData) => {
        return { x: categoryData.dataKey, y: categoryData.dataValue };
      },
    );

    const formatItemPie = salesReport.menuItemGross.map(
      (itemData) => {
        return { x: itemData.dataKey, y: itemData.dataValue };
      },
    );
    console.log(
      salesReport.menuItemGross.map((itemData) => itemData.dataKey),
    );
    return (
      <div>
        <div className="chart-container__full-row">
          ...
          <div className="gross-chart-dimensions">
            <VictoryChart theme={VictoryTheme.material}>
              <VictoryAxis
                tickValues={salesReport.timeSeriesData.map(
                  (timeseries) => timeseries.time,
                )}
                tickFormat={salesReport.timeSeriesData.map(
                  (timeseries) => {
        ...
        <div className="chart-container__columns">
          <div className="itembars-container">
            ...
            >
              <VictoryAxis
                tickValues={salesReport.menuItemGross.map(
                  (datapoint) => datapoint.dataKey,
                )}
                tickFormat={salesReport.menuItemGross.map(
                  (datapoint) => datapoint.dataKey,
                )}
                style={{
                  tickLabels: {
          ...
          <table className="analytics-table">
            ...
            <tbody className="omni-table-body">
              {this.state.allReports &&
                this.renderSaleReportTableToDOM()}
            </tbody>
          </table>
        </div>
      </div>
