...
class TerminalActionMobile extends Component {
  ...
  renderSidebarBody = (sidebarViewState) => {
    if (sidebarViewState === 'category')
      return this.renderItemCategorySelectionButtons();
    if (sidebarViewState === 'items')
      return this.buildItemSelectionMenu();
    if (sidebarViewState === 'tickets')
      return this.buildTicketSelectionMenu();
  };
    ...
    return Object.keys(menuItems).map((category) => {
      if (category === visibleCategory)
        return (
          <div
            key={category}
            className={`${category} touchpad-subcategory`}
          >
            {' '}
        ...
        <tr
          ...
          style={
            ticket._id === activeTicket._id
              ? { backgroundColor: 'rgba(24,123,123,0.25)' }
              : {}
          }
        >
          <td> {ticket.status} </td>
  ...
  render() {
    ...
    return (
      <div className="mobile-action-wrapper">
        <ModalRoot />
        <div
          className={`mobile-action-sidebar${
            this.state.showSidebar ? ' activeSidebarMenu' : ''
          }`}
        >
          <div className="mobile-action-sidebar__header">
            <button
              onClick={() => {
                this.setState({
                  sidebarContent: 'category',
        ...
        <div className="mobile-action__header">
          <button
            ...
            onClick={() => this.props.routeToNode('/omni/terminal')}
          >
