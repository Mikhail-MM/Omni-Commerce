...
class ClockInOutForm extends Component {
  ...
  handleSubmit = (event, token, employeeNumber) => {
    ...
    if (this.props.option === 'Clock In')
      return this.props.clockEmployeeIn(token, employeeNumber);
    if (this.props.option === 'Clock Out')
      return this.props.clockEmployeeOut(token, employeeNumber);
  };
  render() {
    ...
    return (
      <div>
        {this.props.option === 'Clock In' && (
          <form
            onSubmit={(e) =>
              this.handleSubmit(e, token, this.state.employeeNumber)
            }
          >
            <input
