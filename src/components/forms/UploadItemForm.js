...
import {
  modifyOmniTerminalItem,
  createNewMenuItem,
} from '../../actions/terminalItems';
import {
...
const mapDispatchToProps = (dispatch) => ({
  ...
  modifyOmniItem: (token, itemID, data, imageHandler) => {
    dispatch(
      modifyOmniTerminalItem(token, itemID, data, imageHandler),
    );
  },
  modifyEssosItem: (token, itemID, data, imageHandler) => {
    dispatch(
      updateMarketplaceItem(token, itemID, data, imageHandler),
    );
  },
...
class UploadItemForm extends Component {
      ...
      switch (this.props.module) {
        ...
        case 'Essos':
          return this.setState({
            itemName: this.props.modifyItemAttributes.itemName,
            itemPrice: this.props.modifyItemAttributes.itemPrice,
            numberInStock: this.props.modifyItemAttributes
              .numberInStock,
            imageSource: this.props.modifyItemAttributes.imageURL,
      ...
      if (!this.state.tags.includes(tag))
        return (
          ...
          >
            ...
            <div className="tag-label-container">{tag}</div>
          </div>
        );
  ...
  render() {
    return this.props.module === 'Omni' ? (
      <form
        className="omni-item-form-wrapper"
      ...
      >
        ...
        <div>
          ...
          <input
            ...
            onChange={(event) =>
              this.handleChange('itemName', event.target.value)
            }
          />
        </div>
        <div>
          ...
          <input
            ...
            onChange={(event) =>
              this.handleChange('itemPrice', event.target.value)
            }
          />
        </div>
        <div>
        ...
        {this.props.action === 'modify' && (
          <div>
            <button
              className="essos-delete-icon-container"
              onClick={() =>
                this.props.showModal('CONFIRM_DELETE_MODAL', {
                  module: 'Omni',
                  itemData: this.props.modifyItemAttributes,
                })
              }
            >
              {' '}
      ...
      <form
        className="essos-item-form-wrapper"
        onSubmit={(event) => this.handleSubmit(event)}
      >
        ...
        <div className="essos-product-details-modify-column">
          ...
          <div>
            ...
            <input
              ...
              onChange={(event) =>
                this.handleChange('numberInStock', event.target.value)
              }
            />
          </div>
          <div>
