...
const mapDispatchToProps = (dispatch) => ({
  showModal: (modalType, modalRoot) =>
    dispatch(showModal(modalType, modalRoot)),
  followUser: (token, userId, mode) =>
    dispatch(followUser(token, userId, mode)),
});
...
class UserPage extends Component {
  ...
  fetchProfilePageMetadata = (userID) => {
    ...
    if (!this.props.selfProfileView)
      return fetch(
        `/users/essos/getProfileView/${this.props.match.params.id}`,
        {
          headers: {
  ...
  handleFollowRequest = () => {
    ...
    if (
      followContacts.find(
        (user) => user.userId == this.props.match.params.id,
      )
    ) {
  ...
  render() {
    const UserDetailDisplayComponent =
      UserPageComponentMap[this.state.componentView];
      ...
      <div className="user-page-wrapper">
            ...
            <div className="user-social-container">
                  ...
                  <div
                    ...
                    onClick={() =>
                      this.props.showModal(
                        'MODIFY_ESSOS_PROFILE_SETTINGS',
                        { profileData: this.state.fullData },
                      )
                    }
                  >
                    Profile Settings
              ...
              <div className="user-social-stats">
                ...
                <div
                  className="userpage-ticker-container"
                  onClick={() =>
                    this.props.showModal('VIEW_USER_WISHLIST', {
                      wishlistArray: this.state.wishlist,
                      arrayType: 'Wishlist',
                    })
                  }
                >
            ...
            {this.props.selfProfileView ? (
              <div className="user-context-menu-button-container">
                ...
                >
                  <div> My Items </div>
                  <div
                    className={`user-menu-category-button-icon-container ${
                      this.state.componentView === 'USER_MARKET_ITEMS'
                        ? ' active-button-icon-container'
                        : ''
                    }`}
                  >
                ...
                <div
                  ...
                  onClick={() =>
                    this.setState({
                      indexActive: 1,
                      componentView: 'SHIPMENT_REQUESTS',
                    })
                  }
                >
                  <div> My Sales </div>
                  <div
                    className={`user-menu-category-button-icon-container ${
                      this.state.componentView === 'SHIPMENT_REQUESTS'
                        ? ' active-button-icon-container'
                        : ''
                    }`}
                  >
                ...
                <div
                  ...
                  onClick={() =>
                    this.setState({
                      indexActive: 2,
                      componentView: 'PURCHASE_HISTORY',
                    })
                  }
                >
                  ...
                  <div
                    className={`user-menu-category-button-icon-container ${
                      this.state.componentView === 'PURCHASE_HISTORY'
                        ? ' active-button-icon-container'
                        : ''
                    }`}
                  >
                ...
                >
                  ...
                  <div
                    ...
                    onClick={() => {
                      ...
                      this.props.showModal(
                        'DATABASE_INTERFACE_MODAL',
                        { module: 'Essos', action: 'upload' },
                      );
                    }}
                  >
        ...
        {this.state.loading ? (
          ...
          <UserDetailDisplayComponent
            {...this.props}
            selfProfileID={this.state.selfID}
            setItemsLength={this.setItemsLengthAfterLoading}
          />
        )}
      </div>
    );
