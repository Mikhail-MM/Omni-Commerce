...
const mapDispatchToProps = (dispatch) => ({
  ...
  wishlistAction: (token, itemId, mode) =>
    dispatch(addItemToWishlist(token, itemId, mode)),
...
class EssosMarket extends Component {
  ...
  handleWishlistClick = (itemId) => {
    ...
    if (!alreadyInWishlist) {
      return this.props.wishlistAction(token, itemId, 'add');
    } else if (alreadyInWishlist) {
      return this.props.wishlistAction(token, itemId, 'remove');
    }
  };
    ...
    return (
      <div className="ratings-menu-column">
        ...
        <div
          ...
          onClick={() =>
            this.props.showModal('VIEW_REVIEWS_MODAL', {
              reviewArray: reviews,
              arrayType: 'Reviews',
            })
          }
        ...
        <div className="ui_card_mockup">
          <div className="ui_card_image">
            ...
            <div className="card-image__hoverover">
              ...
              {item.reviews &&
                this.generateReviewHoveroverContainer(item.reviews)}
            </div>
          </div>
          <div className="ui_card_content">
            <div className="ui-card-infotext">
              <div className="item-header-container">
                <h3 className="StoreItem-Header-Name">
                  {' '}
                  {item.itemName}{' '}
                </h3>
                <div
                  className={this.handleWishlistIcon(item._id)}
                  onClick={
                    isAuthenticated
                      ? () => this.handleWishlistClick(item._id)
                      : () => {}
                  }
                >
                  <div className="wishlist-icon-message">
                    {' '}
                    Add to Wishlist{' '}
                  </div>
              ...
              {item.followers.length > 0 && (
                <p
                  ...
                  onClick={() =>
                    this.props.showModal('VIEW_USER_SOCIAL_DETAILS', {
                      userArray: item.followers,
                      arrayUserType: 'Wishlisted',
                    })
                  }
                >
                  <span> {item.followers.length} </span> user
                  {item.followers.length > 1 && 's'} want
                  {item.followers.length < 2 && 's'} this item.
                </p>
              )}
            ...
            <div
              ...
              onClick={() =>
                this.props.showModal('CONFIRM_CART_ADDITION', {
                  item: item,
                  renderReviews: this
                    .generateReviewHoveroverContainer,
                })
              }
            >
      ...
      return (
        <div className="cart-item-container-row">
          <div className="cart-item-mini-image-container">
            <img
              alt=""
              className="cart-item-mini-image"
              src={item.imageURL}
            />
          </div>
  ...
  generateJumbotronTargetbox = () => {
    ...
    return this.state.jumbotronItems.map((item, index) => {
      ...
      return (
        <React.Fragment>
          <div
            className={`jumbotarget${targetNo} jumbotron-context-box`}
            onClick={() =>
              this.props.showModal('CONFIRM_CART_ADDITION', {
                item: item,
              })
          ...
          <div
            className={`jumbotarget${targetNo}__companion context-container`}
          >
            <h6> {item.itemName} </h6>
    ...
    return (
      <div className="app-root">
          ...
          <div className="account-control">
            ...
            <div className="my-cart-button">
              <img
                ...
                src="./assets/icons/my-cart.svg"
              />
              <div className="my-cart-dropdown">
                <div className="shopping-cart-dropdown-container">
                  {shoppingCart &&
                    this.generateShoppingCartDropdownContent()}
                  <button
                    onClick={() =>
                      this.props.routeToNode('/essos/mycart')
                    }
                  >
                    {' '}
                    Check Out{' '}
                ...
                <div className="shopping-cart-dropdown-pricing-container"></div>
              </div>
            </div>

            <div
              onClick={() => {
                if (!isAuthenticated || instanceType !== 'Essos') {
                  this.props.showModal('AUTH_FORM_MODAL', {
                    login: true,
                    loginEssos: true,
                  });
                } else if (
                  ...
                  this.props.routeToNode('/essos/profile/');
                }
              }}
            ...
            >
              <img
                ...
                src="./assets/icons/online-store.svg"
              />
              <span> My Shop </span>
            </div>
          </div>
        ...
        <div className="jumbotron">
          <img
            alt=""
            className="jumbotron-greeter"
            src="./assets/store-splash/greeting4.jpg"
          />
          {!this.state.jumboLoading &&
            this.generateJumbotronTargetbox()}
        </div>
